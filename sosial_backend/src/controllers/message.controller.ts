...
import { createMessageService, deleteMessageService, getMessageService } from "../services/message.service";
...
export const getMessageController = async (req: Request, res: Response<responseApi>) => {
    try {
        ...
        const listMessage = messages.map(msg => mapMessage(msg))
...
export const deleteMessageController = async (req: Request, res: Response<responseApi>) => {
    try {
        ...
        await pusher.trigger(`chat-${conversationID}`, 'delete-message', { id: deletedMessage.id })
        return res.status(200).json({
            success: true,
            statusCode: 200,
